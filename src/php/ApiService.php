...
class ApiService
{
	...
	public function request($args, $multipart = false, $signed = true)
	{
		...
		if ($signed) { curl_setopt($ch, CURLOPT_HTTPHEADER, array($oauthHeader));};
		...
		if (isset($data->error)) {
			...
			$uua = $this->endpoint . '?' . http_build_query($args);
			$rrr = '[api] Received error :- ' . $data->error->code . ' : ' . $data->error->info;
			#---------------------
			// $rrr .= ' : ' . json_encode($args);
			// $rrr .= '<br>' . $uua;
			// $rrr .= '<br>' . $oauthHeader;
			#---------------------
			throw new ApiError($rrr);
		}
		/*if (isset($data->error)) {
			throw new ApiError('[api] Received error: ' . $data->error->code . ' : ' . $data->error->info);
		}*/
